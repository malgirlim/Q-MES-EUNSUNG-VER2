...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "예방보전_예방보전계획") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
    ...
    if (req.body.searchKey == "전체") {
      sql =
        ...
        SELECT
          ...
          최소 AS 최소, 최대 AS 최대, 계획일 AS 계획일, 예보일 AS 예보일, 담당자ID AS 담당자ID, 담당자 AS 담당자,
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_PREVENT_PLAN_TB]
          ([PVPL_FACILITY_PK]
          ,[PVPL_DIV]
          ,[PVPL_CONTENT]
          ,[PVPL_HOW]
          ...
          ,[PVPL_REGIST_DT])
        VALUES
          (@설비NO,@구분,@내용,@검사방법,@기준,@단위,@최소,@최대,@계획일,@예보일,@담당자ID,@비고,@등록자,@등록일시)
      `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_PREVENT_PLAN_TB]
          SET 
            [PVPL_FACILITY_PK] = @설비NO
            ,[PVPL_DIV] = @구분
            ,[PVPL_CONTENT] = @내용
            ,[PVPL_HOW] = @검사방법
            ,[PVPL_STAND] = @기준
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_PREVENT_PLAN_TB] WHERE [PVPL_PK] = @key`
        );
