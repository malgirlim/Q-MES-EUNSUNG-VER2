...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "예방보전_설비수리 예보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [WNFCF_PK] AS NO
        ,[WNFCF_FC_FIX_PLAN_PK] AS 참조NO
        ,[WNFCF_DIV] AS 구분
        ,[WNFCF_CONTENT] AS 내용
        ,[WNFCF_NOTE] AS 비고
        ,[WNFCF_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[WNFCF_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_WARNING_FACILITY_FIX_TB]
      ORDER BY [WNFCF_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_WARNING_FACILITY_FIX_TB]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_WARNING_FACILITY_FIX_TB]
          SET 
            [WNFCF_FC_FIX_PLAN_PK] = @참조NO
            ,[WNFCF_DIV] = @구분
            ,[WNFCF_CONTENT] = @내용
            ,[WNFCF_NOTE] = @비고
            ,[WNFCF_REGIST_NM] = @등록자
            ,[WNFCF_REGIST_DT] = @등록일시
          WHERE [WNFCF_PK] = @NO
    `);
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_WARNING_FACILITY_FIX_TB] WHERE [WNFCF_PK] = @key`
        );
