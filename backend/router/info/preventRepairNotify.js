...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "예방보전_설비수리 통보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [NTFCF_PK] AS NO
        ,[NTFCF_FC_FIX_PLAN_PK] AS 참조NO
        ,[NTFCF_DIV] AS 구분
        ,[NTFCF_CONTENT] AS 내용
        ,[NTFCF_NOTE] AS 비고
        ,[NTFCF_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[NTFCF_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_NOTICE_FACILITY_FIX_TB]
      ORDER BY [NTFCF_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_FACILITY_FIX_TB]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_NOTICE_FACILITY_FIX_TB]
          SET 
            [NTFCF_FC_FIX_PLAN_PK] = @참조NO
            ,[NTFCF_DIV] = @구분
            ,[NTFCF_CONTENT] = @내용
            ,[NTFCF_NOTE] = @비고
            ,[NTFCF_REGIST_NM] = @등록자
            ,[NTFCF_REGIST_DT] = @등록일시
          WHERE [NTFCF_PK] = @NO
    `);
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_NOTICE_FACILITY_FIX_TB] WHERE [NTFCF_PK] = @key`
        );
