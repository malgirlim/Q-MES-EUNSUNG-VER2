...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "주문관리_수주대비납품 통보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [NTDL_PK] AS NO
        ,[NTDL_ACCEPT_PK] AS 참조NO
        ,[NTDL_DIV] AS 구분
        ,[NTDL_CONTENT] AS 내용
        ,[NTDL_NOTE] AS 비고
        ,[NTDL_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[NTDL_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_NOTICE_DELIVERY_TB]
      ORDER BY [NTDL_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_DELIVERY_TB]
          ([NTDL_ACCEPT_PK]
          ,[NTDL_DIV]
          ,[NTDL_CONTENT]
          ,[NTDL_NOTE]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_NOTICE_DELIVERY_TB]
          SET 
            [NTDL_ACCEPT_PK] = @참조NO
            ,[NTDL_DIV] = @구분
            ,[NTDL_CONTENT] = @내용
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_NOTICE_DELIVERY_TB] WHERE [NTDL_PK] = @key`
        );
