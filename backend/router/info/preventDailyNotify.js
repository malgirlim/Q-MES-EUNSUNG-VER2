...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "예방보전_일상점검 통보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [NTDISP_PK] AS NO
        ,[NTDISP_DAILY_INSP_PLAN_PK] AS 참조NO
        ,[NTDISP_DIV] AS 구분
        ,[NTDISP_CONTENT] AS 내용
        ,[NTDISP_NOTE] AS 비고
        ,[NTDISP_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[NTDISP_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_NOTICE_DAILY_INSPECT_TB]
      ORDER BY [NTDISP_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_DAILY_INSPECT_TB]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_NOTICE_DAILY_INSPECT_TB]
          SET 
            [NTDISP_DAILY_INSP_PLAN_PK] = @참조NO
            ,[NTDISP_DIV] = @구분
            ,[NTDISP_CONTENT] = @내용
            ,[NTDISP_NOTE] = @비고
            ,[NTDISP_REGIST_NM] = @등록자
            ,[NTDISP_REGIST_DT] = @등록일시
          WHERE [NTDISP_PK] = @NO
    `);
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_NOTICE_DAILY_INSPECT_TB] WHERE [NTDISP_PK] = @key`
        );
