...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "주문관리_수주대비납품 예보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [WNDL_PK] AS NO
        ,[WNDL_ACCEPT_PK] AS 참조NO
        ,[WNDL_DIV] AS 구분
        ,[WNDL_CONTENT] AS 내용
        ,[WNDL_NOTE] AS 비고
        ,[WNDL_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[WNDL_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_WARNING_DELIVERY_TB]
      ORDER BY [WNDL_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_WARNING_DELIVERY_TB]
          SET 
            [WNDL_ACCEPT_PK] = @참조NO
            ,[WNDL_DIV] = @구분
            ,[WNDL_CONTENT] = @내용
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_WARNING_DELIVERY_TB] WHERE [WNDL_PK] = @key`
        );
