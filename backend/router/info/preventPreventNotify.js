...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "예방보전_예방보전 통보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [NTPV_PK] AS NO
        ,[NTPV_PREVENT_PLAN_PK] AS 참조NO
        ,[NTPV_DIV] AS 구분
        ,[NTPV_CONTENT] AS 내용
        ,[NTPV_NOTE] AS 비고
        ,[NTPV_REGIST_NM] AS 등록자
        ,CONVERT(varchar,[NTPV_REGIST_DT],20) AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_NOTICE_PREVENT_TB]
      ORDER BY [NTPV_PK] DESC
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_PREVENT_TB]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_NOTICE_PREVENT_TB]
          SET 
            [NTPV_PREVENT_PLAN_PK] = @참조NO
            ,[NTPV_DIV] = @구분
            ,[NTPV_CONTENT] = @내용
            ,[NTPV_NOTE] = @비고
            ,[NTPV_REGIST_NM] = @등록자
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_NOTICE_PREVENT_TB] WHERE [NTPV_PK] = @key`
        );
