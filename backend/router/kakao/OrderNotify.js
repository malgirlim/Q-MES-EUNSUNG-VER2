  ...
  for (let judge of judge_stand) {
    ...
    if (judge.기능사용 == "ON" && judge.발송시간 == dayjs().format("HH:mm")) {
      ...
      for (let data of send_data) {
        ...
        kakaoSendData.납기경과일 = data.납기경과일 ?? "";
...
async function getSendData() {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        수주NO AS 수주NO
        ,CAST(납기경과일 AS VARCHAR) AS 납기경과일
      ...
      (
        SELECT
          [ACPT_PK] AS 수주NO
          ,DATEDIFF(dd,CONVERT(varchar, GETDATE(), 23),CONVERT(varchar, [ACPT_DELIVERY_DATE], 23)) * (-1) AS 납기경과일
          ,[ACPT_CODE] AS 수주코드
...
async function insertForecastNotify(data) {
  try {
    ...
    await Pool.request()
      ...
      .input("등록자", "시스템" ?? "")
      .input("등록일시", dayjs().format("YYYY-MM-DD HH:mm:ss"))
      .query(`INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_DELIVERY_TB]
          ([NTDL_ACCEPT_PK]
          ,[NTDL_DIV]
          ,[NTDL_CONTENT]
          ,[NTDL_NOTE]
