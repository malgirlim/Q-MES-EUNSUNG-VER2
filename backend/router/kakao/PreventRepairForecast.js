...
const options = {
  uri: "http://aligo.qmes.co.kr/q-api/kakao/preventrepairforecast/send",
  method: "POST",
  body: kakaoSendData,
  json: true,
};
...
async function getJudgeStand() {
  try {
    ...
    const result = await Pool.request().query(`
      ...
      WHERE [ALST_DIV] = 'PreventRepairForecast'
    `);
    ...
    const result = await Pool.request().query(`
      ...
      (
        SELECT
          ...
          ,DATEDIFF(dd,CONVERT(varchar, GETDATE(), 23),CONVERT(varchar, [FCFPL_DATE], 23)) AS 잔여일
          ,DATEDIFF(dd,CONVERT(varchar, GETDATE(), 23),CONVERT(varchar, [FCFPL_WARN_DATE], 23)) AS 예보잔여일
          ,[FCFPL_USER_ID] AS 담당자ID
          ,(SELECT [USER_NAME] FROM [QMES2022].[dbo].[MASTER_USER_TB] WHERE [USER_ID] = [FCFPL_USER_ID]) AS 담당자
          ,(SELECT [USER_RANK] FROM [QMES2022].[dbo].[MASTER_USER_TB] WHERE [USER_ID] = [FCFPL_USER_ID]) AS 직급
          ,(SELECT COUNT(*) FROM [QMES2022].[dbo].[MANAGE_FACILITY_FIX_TB] WHERE [FCFIX_FACILITY_FIX_PLAN_PK] = [FCFPL_PK]) AS 계획완료수
          ,[FCFPL_NOTE] AS 비고
...
async function insertForecastNotify(data) {
  try {
    ...
    await Pool.request()
      ...
      .input("등록자", "시스템" ?? "")
      .input("등록일시", dayjs().format("YYYY-MM-DD HH:mm:ss"))
      .query(`INSERT INTO [QMES2022].[dbo].[MANAGE_WARNING_FACILITY_FIX_TB]
          ([WNFCF_FC_FIX_PLAN_PK]
          ,[WNFCF_DIV]
          ,[WNFCF_CONTENT]
          ,[WNFCF_NOTE]
          ,[WNFCF_REGIST_NM]
