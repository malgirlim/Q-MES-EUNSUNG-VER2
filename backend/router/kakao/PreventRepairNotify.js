...
const options = {
  uri: "http://aligo.qmes.co.kr/q-api/kakao/preventrepairnotify/send",
  method: "POST",
  body: kakaoSendData,
  json: true,
};
...
async function getJudgeStand() {
  try {
    ...
    const result = await Pool.request().query(`
      ...
      WHERE [ALST_DIV] = 'PreventRepairNotify'
    `);
...
async function insertForecastNotify(data) {
  try {
    ...
    await Pool.request()
      ...
      .input("등록자", "시스템" ?? "")
      .input("등록일시", dayjs().format("YYYY-MM-DD HH:mm:ss"))
      .query(`INSERT INTO [QMES2022].[dbo].[MANAGE_NOTICE_FACILITY_FIX_TB]
          ([NTFCF_FC_FIX_PLAN_PK]
          ,[NTFCF_DIV]
          ,[NTFCF_CONTENT]
          ,[NTFCF_NOTE]
          ,[NTFCF_REGIST_NM]
