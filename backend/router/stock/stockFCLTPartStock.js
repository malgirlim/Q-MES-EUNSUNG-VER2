...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "재고관리_설비부품 재고현황") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
      ...
      (
        ...
        (
          ...
          (
            ...
            GROUP BY [FPRC_FACILITY_PART_PK]
          ) AS 입고_MIDDLE
          LEFT JOIN
          (
            SELECT
            ...
            GROUP BY [FPRL_FACILITY_PART_PK]
          ) AS 출고_MIDDLE ON 출고_MIDDLE.설비부품NO = 입고_MIDDLE.설비부품NO
          ...
          (
            ...
            AND CONVERT(VARCHAR, CONVERT(datetime, [FPRL_DT]), 12) < '000101'
            GROUP BY [FPRL_FACILITY_PART_PK]
          ) AS 출고_기초재고_MIDDLE ON 출고_기초재고_MIDDLE.설비부품NO = 입고_기초재고_MIDDLE.설비부품NO
        ) AS RESULT_MIDDLE ON RESULT_MIDDLE.설비부품NO = [FCPT_PK]
        GROUP BY [FCPT_PK],[FCPT_CLIENT_PK],[FCPT_FACILITY_PK],[FCPT_DIV],[FCPT_PRODUCT_NUM],[FCPT_NAME],[FCPT_CAR],[FCPT_SIZE],[FCPT_UNIT],[FCPT_SAFE],[FCPT_COST]
      ) AS RESULT
      ORDER BY 설비부품NO DESC
    `);
