  ...
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        ...
        ,ITEM_PROCESS.품목구분 AS 품목구분
        ,ITEM_PROCESS.품번 AS 품번
  ...
  try {
    ...
    if (req.body.searchKey == "전체") {
      sql =
        `
        SELECT
          NO AS NO, 생산실적NO AS 생산실적NO, 품목재공NO AS 품목재공NO, 코드 AS 코드, 품목구분 AS 품목구분, 품번 AS 품번,
          품명 AS 품명, 규격 AS 규격, 단위 AS 단위, 수량 AS 수량, 사용일시 AS 사용일시,
