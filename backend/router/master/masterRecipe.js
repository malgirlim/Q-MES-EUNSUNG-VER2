...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "기준정보_레시피관리") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [RECP_PK] AS NO
        ,[RECP_ITEM_PK] AS 품목NO
        ,ITEM.구분 AS 품목구분
        ,ITEM.품번 AS 품번
        ,ITEM.품명 AS 품명
        ,[RECP_IMAGE] AS 사진
        ,[RECP_NOTE] AS 비고
        ,[RECP_REGIST_NM] AS 등록자
        ,[RECP_REGIST_DT] AS 등록일시
      FROM [QMES2022].[dbo].[MASTER_RECIPE_TB]
      LEFT JOIN
      (
        SELECT
          [ITEM_PK] AS NO
      ...
      ) AS ITEM ON ITEM.NO = [RECP_ITEM_PK]
      ORDER BY [RECP_PK] DESC
    `);
    ...
    if (req.body.searchKey == "전체") {
      sql =
        ...
        SELECT
          ...
          사진 AS 사진, 비고 AS 비고, 등록자 AS 등록자, 등록일시 AS 등록일시
        FROM(
          SELECT
          [RECP_PK] AS NO
          ,[RECP_ITEM_PK] AS 품목NO
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MASTER_RECIPE_TB]
          ([RECP_ITEM_PK]
          ,[RECP_IMAGE]
          ,[RECP_NOTE]
          ,[RECP_REGIST_NM]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MASTER_RECIPE_TB]
          SET 
            [RECP_ITEM_PK] = @품목NO
            ,[RECP_IMAGE] = @사진
            ,[RECP_NOTE] = @비고
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MASTER_RECIPE_TB] WHERE [RECP_PK] = @key`
        );
...
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, done) => {
      done(null, "uploads/master/recipe");
    },
