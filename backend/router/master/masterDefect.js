...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "기준정보_불량내용코드관리") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MASTER_DEFECT_TB]
          ([DEFT_CODE]
          ,[DEFT_DIV]
          ,[DEFT_NAME]
          ,[DEFT_CONTENT]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MASTER_DEFECT_TB]
          SET 
            [DEFT_CODE] = @코드
            ,[DEFT_DIV] = @구분
            ,[DEFT_NAME] = @불량명
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MASTER_DEFECT_TB] WHERE [DEFT_PK] = @key`
        );
