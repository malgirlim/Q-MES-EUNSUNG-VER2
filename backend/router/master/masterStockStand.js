...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "기준정보_재고기준정보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
  ...
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        ...
        ,[STST_CONTENT] AS 내용
        ,[STST_NOTE] AS 비고
        ,[STST_REGIST_NM] AS 등록자
        ,[STST_REGIST_DT] AS 등록일시
      FROM [QMES2022].[dbo].[MASTER_STOCK_STAND_TB]
      LEFT JOIN
      (
        SELECT
          [ITEM_PK] AS NO
      ...
      ) AS ITEM ON ITEM.NO = [STST_ITEM_PK]
      ORDER BY [STST_PK] DESC
    `);
  ...
  try {
    ...
    if (req.body.searchKey == "전체") {
      sql =
        `
        SELECT
          NO AS NO, 품목NO AS 품목NO, 품목구분 AS 품목구분, 품번 AS 품번, 품명 AS 품명,
          구분 AS 구분, 내용 AS 내용, 비고 AS 비고, 등록자 AS 등록자, 등록일시 AS 등록일시
        FROM(
          SELECT
            [STST_PK] AS NO
            ,[STST_ITEM_PK] AS 품목NO
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MASTER_STOCK_STAND_TB]
          ([STST_ITEM_PK]
          ,[STST_DIV]
          ,[STST_CONTENT]
          ,[STST_NOTE]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MASTER_STOCK_STAND_TB]
          SET 
            [STST_ITEM_PK] = @품목NO
            ,[STST_DIV] = @구분
            ,[STST_CONTENT] = @내용
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MASTER_STOCK_STAND_TB] WHERE [STST_PK] = @key`
        );
