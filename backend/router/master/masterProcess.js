...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "기준정보_공정관리") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.post("/insert", async (req, res) => {
  try {
    ...
    await Pool.request()
      ...
      .input("공정명", req.body.data.공정명 ?? "")
      .input("내용", req.body.data.내용 ?? "")
      .input("설비", req.body.data.설비 ?? "")
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MASTER_PROCESS_TB]
          ([PRCS_CODE]
          ,[PRCS_DIV]
          ,[PRCS_NAME]
          ,[PRCS_CONTENT]
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      await Pool.request()
        ...
        .input("공정명", req.body.data[i].공정명 ?? "")
        .input("내용", req.body.data[i].내용 ?? "")
        .input("설비", req.body.data[i].설비 ?? "")
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MASTER_PROCESS_TB]
          SET 
            [PRCS_CODE] = @코드
            ,[PRCS_DIV] = @구분
            ,[PRCS_NAME] = @공정명
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MASTER_PROCESS_TB] WHERE [PRCS_PK] = @key`
        );
