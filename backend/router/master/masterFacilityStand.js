...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "기준정보_설비기준정보") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [FCST_PK] AS NO
        ,[FCST_FACILITY_PK] AS 설비NO
        ,FACILITY.설비명 AS 설비명
        ,[FCST_DIV] AS 구분
        ,[FCST_CONTENT] AS 내용
        ,[FCST_NOTE] AS 비고
        ,[FCST_REGIST_NM] AS 등록자
        ,[FCST_REGIST_DT] AS 등록일시
      FROM [QMES2022].[dbo].[MASTER_FACILITY_STAND_TB]
      LEFT JOIN
      (
        SELECT
          [FCLT_PK] AS NO
      ...
      ) AS FACILITY ON FACILITY.NO = [FCST_FACILITY_PK]
      ORDER BY [FCST_PK] DESC
    `);
  ...
  try {
    ...
    if (req.body.searchKey == "전체") {
      sql =
        `
        SELECT
          NO AS NO, 설비NO AS 설비NO, 설비명 AS 설비명, 구분 AS 구분, 내용 AS 내용,
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MASTER_FACILITY_STAND_TB]
          ([FCST_FACILITY_PK]
          ,[FCST_DIV]
          ,[FCST_CONTENT]
          ,[FCST_NOTE]
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MASTER_FACILITY_STAND_TB]
          SET 
            [FCST_FACILITY_PK] = @설비NO
            ,[FCST_DIV] = @구분
            ,[FCST_CONTENT] = @내용
            ,[FCST_NOTE] = @비고
            ,[FCST_REGIST_NM] = @등록자
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MASTER_FACILITY_STAND_TB] WHERE [FCST_PK] = @key`
        );
