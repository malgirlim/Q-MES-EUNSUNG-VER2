...
const logSend = async (type, ct, amount, user) => {
  ...
  await Pool.request() // 로그기록 저장
    .input("type", type)
    .input("menu", "품질관리_검사성적서") // ############ *중요* 이거 메뉴 이름 바꿔야함 !! #########
...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await Pool.request().query(`
      SELECT
        [QARP_PK] AS NO
        ,CONVERT(VARCHAR, [QARP_DATE], 23) AS 검사일
        ,[QARP_DIV] AS 구분
        ,[QARP_ITEM_PK] AS 품목NO
        ,ITEM.품번 AS 품번
        ,ITEM.품목구분 AS 품목구분
        ,ITEM.품명 AS 품명
        ,ITEM.규격 AS 규격
        ,ITEM.단위 AS 단위
        ,[QARP_AMOUNT] AS 검사수량
        ,[QARP_SAMPLE_AMT] AS 샘플수량
        ,[QARP_RESULT] AS 결과
        ,[QARP_IMAGE] AS 이미지
        ,[QARP_NOTE] AS 비고
        ,[QARP_REGIST_NM] AS 등록자
        ,[QARP_REGIST_DT] AS 등록일시
      FROM [QMES2022].[dbo].[MANAGE_QUALITY_REPORT_TB]
      LEFT JOIN
      (
        SELECT
          [ITEM_PK] AS NO
      ...
      ) AS ITEM ON ITEM.NO = [QARP_ITEM_PK]
      ORDER BY [QARP_PK] DESC
    `);
  ...
  try {
    ...
    if (req.body.searchKey == "전체") {
      sql =
        `
        SELECT
          NO AS NO, 검사일 AS 검사일, 구분 AS 구분, 품목NO AS 품목NO, 품번 AS 품번, 품목구분 AS 품목구분, 품명 AS 품명,
          규격 AS 규격, 단위 AS 단위, 검사수량 AS 검사수량, 샘플수량 AS 샘플수량, 결과 AS 결과, 이미지 AS 이미지,
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        INSERT INTO [QMES2022].[dbo].[MANAGE_QUALITY_REPORT_TB]
          ([QARP_DATE]
          ,[QARP_DIV]
          ,[QARP_ITEM_PK]
          ,[QARP_AMOUNT]
          ...
          ,[QARP_REGIST_DT])
        VALUES
          (@검사일,@구분,@품목NO,@검사수량,@샘플수량,@결과,@이미지,@비고,@등록자,@등록일시)
    `);
  ...
  try {
    ...
    await Pool.request()
      ...
      .input(
        ...
        UPDATE [QMES2022].[dbo].[MANAGE_QUALITY_REPORT_TB]
          SET
            [QARP_DATE] = @검사일
            ,[QARP_DIV] = @구분
            ,[QARP_ITEM_PK] = @품목NO
            ,[QARP_AMOUNT] = @검사수량
            ,[QARP_SAMPLE_AMT] = @샘플수량
            ,[QARP_RESULT] = @결과
            ,[QARP_IMAGE] = @이미지
            ,[QARP_NOTE] = @비고
            ,[QARP_REGIST_NM] = @등록자
  ...
  try {
    ...
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await Pool.request()
        ...
        .query(
          `DELETE FROM [QMES2022].[dbo].[MANAGE_QUALITY_REPORT_TB] WHERE [QARP_PK] = @key`
        );
...
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, done) => {
      done(null, "uploads/quality/report");
    },
