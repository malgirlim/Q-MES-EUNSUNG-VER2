...
import { MonitorKPIFacilityRate } from "../../interfaces/menu/monitorInterface";

// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterClientDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MonitoringKPIFacilityRate; //권한레벨
...
// dataManager 만들기
const url = "/api/monitor/kpi/facilityrate";
const dataManager = useSendApi<MonitorKPIFacilityRate>(
  url,
  currentPage,
  rowsPerPage
);
                    ...
                    {{
                      Number(
                        (dataManager.dataSearchAll.value.reduce(
                          (sum, data) =>
                            Number(sum) +
                            Number(data.가동률) / Number(data.목표),
                          0
                        ) /
                          dataManager.dataSearchAll.value.length) *
                          100
