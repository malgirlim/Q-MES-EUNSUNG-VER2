...
import MTTRChart_bar_bar from "../../components/Common/Monitoring/MTTRChart_bar_bar.vue";

// API 보내는 함수 및 인터페이스 불러오기
import { useSendApi } from "../../composables/useSendApi";
import { MonitorMTTR } from "../../interfaces/menu/monitorInterface";

// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterClientDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MonitoringMTTR; //권한레벨
...
// dataManager 만들기
const url = "/api/monitor/mttr";
const dataManager = useSendApi<MonitorMTTR>(url, currentPage, rowsPerPage);
...
const table_setting = {
  ...
  항목2: { name: "설비수리시간", style: "width: 50px; text-align: center;" },
  항목3: { name: "수리내역건수", style: "width: 50px; text-align: center;" },
  항목4: { name: "MTTR", style: "width: 50px; text-align: center;" },
                    ...
                    {{
                      Number(
                        dataManager.dataSearchAll.value.reduce(
                          (sum, data) => Number(sum) + Number(data.MTTR),
                          0
      ...
      <div class="col-span-3 p-5 bg-white rounded rounded-md">
        ...
        <div style="height: 270px">
          <MTTRChart_bar_bar
            ...
            :dataset1_label="'MTTR'"
            :dataset1_data="
              dataManager.dataSearchAll.value.map((data) => data.MTTR)
            "
            :dataset2_label="'목표'"
            :dataset2_data="
