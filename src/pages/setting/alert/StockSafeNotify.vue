...
const user_level = proxy.gstate.level.AlertStockSafeNotify; //권한레벨
...
onMounted(async () => {
  ...
  await alertSetting.searchDatas("전체기간", "구분", "StockSafeNotify", "", ""); // 설정 데이터 불러오기
...
const setInsertRuleModal = (value: boolean) => {
  ...
  alertRuleModalData = { 구분: "StockSafeNotify", 설비NO: 0 }; // 초기화
};
// 발송조건 수정 Modal
  ...
  <div v-if="user_level >= 2">
    ...
    <div class="mt-5 grid grid-cols-12 gap-1">
      <div class="col-span-2 border-r-2 border-slate-200" style="height: 640px">
        <div><AlertSelect 안전재고미달통보_variant="primary" /></div>
      </div>
