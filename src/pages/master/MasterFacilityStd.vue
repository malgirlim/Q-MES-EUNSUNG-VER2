...
// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterFacilityStdDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MasterFacilityStd; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  facilitystand_facility.loadDatas(); // 거래처 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/master/facilitystand";
const dataManager = useSendApi<MasterFacilityStand>(
  url,
  currentPage,
  rowsPerPage
);
...
// 추가 데이터
const url_facilitystand_facility = "/api/master/facilitystand/facility";
const facilitystand_facility = useSendApi<MasterFacility>(
  url_facilitystand_facility,
  currentPage,
  rowsPerPage
);
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterFacilityStand;
...
const insertDataFunction = async () => {
  set_설비.value = insertModalData.설비NO;
  await insert_check();
  if (pass_flag == false) {
...
let editModalData: MasterFacilityStand; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["설비명", "구분", "내용", "비고"],
    type: "json",
    documentTitle: "기준정보 > 설비기준정보",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_설비기준정보_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_설비기준정보.xlsx"; // 엑셀 양식주소
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.설비NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in facilitystand_facility.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div v-if="set_설비 == ''" class="text-danger text-xs mt-1">
                설비가 선택되지 않았습니다.
              </div>
              <div class="mt-3">
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.설비NO">
                  <option :value="editModalData.설비NO" selected>
                    {{
                      facilitystand_facility.dataAll.value.filter(
