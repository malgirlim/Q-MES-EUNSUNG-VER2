...
const user_level = proxy.gstate.level.MasterRecipe; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  recipe_product.loadDatas(); // 거래처 데이터 불러오기
});
...
const recipe_product = useSendApi<MasterProduct>(
  url_recipe_product,
  currentPage,
  rowsPerPage
);
...
const table_setting = {
  ...
  항목5: { name: "비고", style: "width: 50px; text-align: center;" },
...
const insertDataFunction = async () => {
  ...
  set_사진.value = insertModalData.사진;
  await insert_check();
  if (pass_flag == false) {
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["품목구분", "품번", "품명", "비고"],
    type: "json",
    documentTitle: "기준정보 > 레시피 관리",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_레시피관리_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_레시피관리.xlsx"; // 엑셀 양식주소
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.품목NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in recipe_product.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.품목NO">
                  <option :value="editModalData.품목NO" selected>
                    {{
                      recipe_product.dataAll.value.filter(
