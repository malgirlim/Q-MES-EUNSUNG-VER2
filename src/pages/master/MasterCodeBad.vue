...
const user_level = proxy.gstate.level.MasterCodeBad; //권한레벨
...
// dataManager 만들기
const url = "/api/master/defect";
const dataManager = useSendApi<MasterDefect>(url, currentPage, rowsPerPage);
...
const insert_check = () => {
  ...
  if (set_불량명.value != null && set_불량명.value != "") {
    insertModalData.불량명 = set_불량명.value;
  } else {
    set_불량명.value = "";
    pass_flag = false;
  }
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterDefect;
...
let editModalData: MasterDefect; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["코드", "구분", "불량명", "내용"],
    type: "json",
    documentTitle: "기준정보 > 불량코드 관리",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_불량코드관리_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_불량코드관리.xlsx"; // 엑셀 양식주소
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">불량명</FormLabel
                ><label class="text-danger"><sup>*</sup></label>
                <FormInput
                  id="vertical-form-6"
                  type="text"
              ...
              <div v-if="set_불량명 == ''" class="text-danger text-xs mt-1">
                불량명이 입력되지 않았습니다.
              </div>
              <div class="mt-3">
