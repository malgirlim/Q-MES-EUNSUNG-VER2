...
const user_level = proxy.gstate.level.MasterStockStd; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  stockstand_product.loadDatas(); // 거래처 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/master/stockstand";
const dataManager = useSendApi<MasterStockStand>(url, currentPage, rowsPerPage);

// 추가 데이터
const url_stockstand_product = "/api/master/stockstand/product";
const stockstand_product = useSendApi<MasterProduct>(
  url_stockstand_product,
  currentPage,
  rowsPerPage
);
...
const table_setting = {
  ...
  항목5: { name: "내용", style: "width: 100px; text-align: center;" },
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterStockStand;
...
let editModalData: MasterStockStand; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["품목구분", "품번", "품명", "구분", "내용"],
    type: "json",
    documentTitle: "기준정보 > 재고기준정보",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_재고기준정보" + dayjs().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_재고기준정보.xlsx"; // 엑셀 양식주소
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.품목NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in stockstand_product.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div v-if="set_내용 == ''" class="text-danger text-xs mt-1">
                내용이 입력되지 않았습니다.
              </div>
            </div>
          </Tab.Panel>
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.품목NO">
                  <option :value="editModalData.품목NO" selected>
                    {{
                      stockstand_product.dataAll.value.filter(
