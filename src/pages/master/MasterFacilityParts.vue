...
const user_level = proxy.gstate.level.MasterFacilityParts; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  facilitypart_client.loadDatas(); // 거래처 데이터 불러오기
  facilitypart_facility.loadDatas(); // 설비 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/master/facilitypart";
const dataManager = useSendApi<MasterFacilityPart>(
  url,
  currentPage,
  rowsPerPage
);
...
// 추가 데이터
// 거래처 데이터
const url_facilitypart_client = "/api/master/facilitypart/client";
const facilitypart_client = useSendApi<MasterClient>(
  url_facilitypart_client,
  currentPage,
  rowsPerPage
);
// 설비 데이터
const url_facilitypart_facility = "/api/master/facilitypart/facility";
const facilitypart_facility = useSendApi<MasterFacility>(
  url_facilitypart_facility,
  currentPage,
  rowsPerPage
);
...
const table_setting = {
  ...
  항목4: { name: "품명", style: "width: 200px; text-align: center;" },
  항목5: { name: "규격", style: "width: 100px; text-align: center;" },
...
const insert_check = () => {
  ...
  if (set_설비명.value != null && set_설비명.value != "") {
    insertModalData.설비명 = set_설비명.value;
  } else {
    set_설비명.value = "";
    pass_flag = false;
  }
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterFacilityPart;
...
let editModalData: MasterFacilityPart; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["설비명", "구분", "품번", "품명", "규격", "단위"],
    type: "json",
    documentTitle: "기준정보 > 설비부품 관리",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_설비부품관리_" + dayjs().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_설비부품관리.xlsx"; // 엑셀 양식주소
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-3">설비명</FormLabel
              ...
              <div v-if="set_설비명 == ''" class="text-danger text-xs mt-1">
                설비명이 입력되지 않았습니다.
              </div>
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.거래처NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in facilitypart_client.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.설비NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in facilitypart_facility.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">차종</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.거래처NO">
                  ...
                  >
                    {{
                      facilitypart_client.dataAll.value.filter(
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.설비NO">
                  ...
                  >
                    {{
                      facilitypart_facility.dataAll.value.filter(
