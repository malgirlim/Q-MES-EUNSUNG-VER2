...
const user_level = proxy.gstate.level.MasterBOM; //권한레벨
...
// dataManager 만들기
const url = "/api/master/bom/product";
const dataManager = useSendApi<MasterProduct>(url, currentPage, rowsPerPage);

// 추가 데이터
const url_bom = "/api/master/bom";
const bom = useSendApi<MasterBom>(url_bom, currentPage, rowsPerPage);
// 테이블항목 설정 및 가로크기 조정
const table_setting = {
...
const table_setting_bom = {
  체크박스: { name: "체크박스", style: "width: 50px" },
  순번: { name: "순번", style: "width: 50px; text-align: center;" },
  항목1: { name: "투입자재구분", style: "width: 50px; text-align: center;" },
  항목2: { name: "투입자재품번", style: "width: 150px; text-align: center;" },
  항목3: { name: "투입자재품명", style: "width: 50px; text-align: center;" },
  항목4: { name: "투입량", style: "width: 50px; text-align: center;" },
...
watch([radioSelect], (newValue, oldValue) => {
  ...
  bom.searchDatas("", "품목NO", radioSelect.value, "", "");
});
...
const insert_check = () => {
  ...
  if (set_투입자재.value != null && set_투입자재.value != "") {
    insertModalData.투입자재 = set_투입자재.value;
  } else {
    set_투입자재.value = "";
    pass_flag = false;
  }

  if (set_투입량.value != null && set_투입량.value != "") {
    insertModalData.투입량 = set_투입량.value;
  } else {
    set_투입량.value = "";
    pass_flag = false;
  }
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterBom;
const insertModal = ref(false);
const setInsertModal = (value: boolean) => {
  if (user_level >= 3) {
    if (radioSelect.value != null) {
      set_투입자재.value = null;
      set_투입량.value = null;
      insertModal.value = value;
      insertModalData = {}; // 변수 초기화
      insertModalData.품목NO = radioSelect.value;
    } else {
      toast.info(
        "선택된 데이터가 없습니다.\nBOM 구성할 데이터를 선택해주세요."
      );
...
const insertDataFunction = async () => {
  set_투입자재.value = insertModalData.투입자재NO;
  await insert_check();
  if (pass_flag == false) {
    ...
    await bom.searchDatas("", "품목NO", radioSelect.value, "", "");
...
const editDataFunction = async () => {
  await bom.editData(editModalData);
  bom.searchDatas("", "품목NO", radioSelect.value, "", "");
};
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_BOM관리_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_BOM관리.xlsx"; // 엑셀 양식주소
...
<template>
      ...
      <div class="grid grid-cols-12 gap-1 mt-1">
        ...
        <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
          ...
          >
              ...
              >
                <Table.Tr>
                  <Table.Th
                    ...
                    id="checkbox"
                    :style="table_setting_bom.체크박스.style"
                  >
                    <Input
                  ...
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.순번.style"
                  >
                    순번
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.항목1.style"
                  >
                    {{ table_setting_bom.항목1.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.항목2.style"
                  >
                    {{ table_setting_bom.항목2.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.항목3.style"
                  >
                    {{ table_setting_bom.항목3.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.항목4.style"
                  >
                    {{ table_setting_bom.항목4.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting.상세보기.style"
                  >
                    {{ table_setting_bom.상세보기.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap font-bold"
                    :style="table_setting_bom.편집.style"
                  >
                    {{ table_setting_bom.편집.name }}
                  </Table.Th>
                </Table.Tr>
              </Table.Thead>
              ...
              <Table.Tbody style="position: relative; z-index: 1">
                ...
                >
                  ...
                  <Table.Td
                    ...
                    :style="table_setting_bom.항목1.style"
                  >
                    <div>{{ todo[table_setting_bom.항목1.name] }}</div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]"
                    :style="table_setting_bom.항목2.style"
                  >
                    <div>{{ todo[table_setting_bom.항목2.name] }}</div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]"
                    :style="table_setting_bom.항목3.style"
                  >
                    <div>{{ todo[table_setting_bom.항목3.name] }}</div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center bg-white border-b-0 dark:bg-darkmode-600 shadow-[20px_3px_20px_#0000000b]"
                    :style="table_setting_bom.항목4.style"
                  >
                    <div>{{ todo[table_setting_bom.항목4.name] }}</div>
                  </Table.Td>
                  <Table.Td
                    ...
                    :style="table_setting_bom.상세보기.style"
                  >
            ...
            <div
              class="text-center mt-20"
              v-if="bom.dataCount.value == 0 && radioSelect != null"
            >
              저장된 데이터가 없습니다.
            </div>
            <div class="text-center mt-20" v-if="radioSelect == null">
              <div>선택된 데이터가 없습니다.</div>
              <div class="mt-3">BOM 구성할 품목을 선택해주세요.</div>
            </div>
          </div>
        </div>
        <!-- END: Data List -->
  ...
  <Dialog size="md" :open="insertModal">
    <Dialog.Panel class="p-10 text-center">
      <!--추가 Modal 내용 시작-->
      <div class="mb-5" style="font-weight: bold">투입자재 등록</div>
      <div style="text-align: left">
        <div class="mt-3">
          <FormLabel htmlFor="vertical-form-2">투입자재</FormLabel
          ><label class="text-danger"><sup>*</sup></label>
          <select v-tom v-model="insertModalData.투입자재NO">
            <option value="" selected>--- 목록에서 선택 ---</option>
            <option
              v-for="todo in dataManager.dataAll.value"
              :key="todo.NO"
              :value="todo.NO"
            >
              {{ todo.품번 }} # 품명:{{ todo.품명 }} # 단위:{{ todo.단위 }}
            </option>
          </select>
        </div>
        <div v-if="set_투입자재 == ''" class="text-danger text-xs mt-1">
          투입자재가 선택되지 않았습니다.
        </div>
        <div class="mt-3">
          <FormLabel htmlFor="vertical-form-3">투입량</FormLabel
          ><label class="text-danger"><sup>*</sup></label>
          <FormInput
            id="vertical-form-3"
            type="text"
        ...
        <div v-if="set_투입량 == ''" class="text-danger text-xs mt-1">
          투입량이 입력되지 않았습니다.
        </div>
        <div class="mt-3">
  ...
  <Dialog size="md" :open="editModal">
    <Dialog.Panel class="p-10 text-center">
      ...
      <div style="text-align: left">
        <div class="mt-3">
          <FormLabel htmlFor="vertical-form-2">투입자재</FormLabel>
          <FormSelect v-model="editModalData.투입자재NO">
            <option
              v-if="editModalData.투입자재NO"
              :value="editModalData.투입자재NO"
              selected
            >
              {{
                dataManager.dataAll.value.filter(
                  (c) => c.NO == editModalData.투입자재NO
                )[0].거래처명
              }}
              # 품명:{{
                dataManager.dataAll.value.filter(
