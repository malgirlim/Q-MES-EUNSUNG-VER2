...
const dataManager = useSendApi<MasterProcess>(url, currentPage, rowsPerPage);
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterProcess;
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_공정관리_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              ...
              <div v-if="set_공정명 == ''" class="text-danger text-xs mt-1">
                공정명이 입력되지 않았습니다.
              </div>
              <div class="mt-3">
