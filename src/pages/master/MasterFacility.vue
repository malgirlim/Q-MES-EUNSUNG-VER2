...
// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterFacilityDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MasterFacility; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  facility_client.loadDatas(); // 거래처 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/master/facility";
const dataManager = useSendApi<MasterFacility>(url, currentPage, rowsPerPage);

// 추가 데이터
const url_facility_client = "/api/master/facility/client";
const facility_client = useSendApi<MasterClient>(
  url_facility_client,
  currentPage,
  rowsPerPage
);
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterFacility;
...
let editModalData: MasterFacility; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["라인", "설비명", "규격"],
    type: "json",
    documentTitle: "기준정보 > 설비 관리",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_설비관리_" + dayjs().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_설비관리.xlsx"; // 엑셀 양식주소
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-3">라인</FormLabel>
                <FormInput
                  id="vertical-form-3"
                  type="text"
                  v-model="insertModalData.라인"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.거래처NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in facility_client.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div>
                <FormLabel htmlFor="vertical-form-2">구입일</FormLabel>
                <FormInput
                  id="vertical-form-2"
                  type="date"
                  v-model="insertModalData.구입일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">금액</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">장소</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="insertModalData.장소"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
            ...
            <div style="text-align: left">
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.거래처NO">
                  ...
                  >
                    {{
                      facility_client.dataAll.value.filter(
      ...
      <div class="p-5 text-center">
        ...
        <div class="mt-5 mb-5">
          <img
            class="mx-auto"
            :src="
              '../../../backend/uploads/master/facility/' + editModalData.사진
            "
          />
        </div>
        <a
