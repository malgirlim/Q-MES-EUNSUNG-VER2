...
// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterCodeNonOPDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MasterCodeNonOP; //권한레벨
...
// dataManager 만들기
const url = "/api/master/nonwork";
const dataManager = useSendApi<MasterNonWork>(url, currentPage, rowsPerPage);
...
const insert_check = () => {
  ...
  if (set_비가동명.value != null && set_비가동명.value != "") {
    insertModalData.비가동명 = set_비가동명.value;
  } else {
    set_비가동명.value = "";
    pass_flag = false;
  }
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MasterNonWork;
...
let editModalData: MasterNonWork; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["코드", "구분", "비가동명", "내용"],
    type: "json",
    documentTitle: "기준정보 > 비가동코드 관리",
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_비가동코드관리.xlsx"; // 엑셀 양식주소
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-3">코드</FormLabel
              ...
              <div v-if="set_코드 == ''" class="text-danger text-xs mt-1">
                코드가 입력되지 않았습니다.
              </div>
              <div class="mt-3">
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">비가동명</FormLabel
                ><label class="text-danger"><sup>*</sup></label>
                <FormInput
                  id="vertical-form-6"
                  type="text"
              ...
              <div v-if="set_비가동명 == ''" class="text-danger text-xs mt-1">
                비가동명이 입력되지 않았습니다.
              </div>
              <div class="mt-3">
