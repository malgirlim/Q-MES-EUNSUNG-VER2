...
onMounted(async () => {
  ...
  stock_modal_item.loadDatas(); // 품목 데이터 불러오기
  setTimeout(() => {
    menu_fix.value = ".";
  }, 500);
});
...
// dataManager 만들기
const url = "/api/stock/stock/raw";
const dataManager = useSendApi<StockStock>(url, currentPage, rowsPerPage);

// 재고관리 - 입고 기능 가져오기
const url_receive_raw = "/api/stock/receive/raw";
const receive_raw = useSendApi<StockItemReceive>(
  url_receive_raw,
  ref(1),
  ref(10)
);
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: StockItemReceive;
const insertModal = ref(false);
const setInsertModal = (value: boolean) => {
  insertModal.value = value;
  insertModalData = {
    구분: "모바일 입고",
    입고일시: dayjs().format("YYYY-MM-DD HH:mm:ss"),
    유효일자: dayjs().add(1, "month").format("YYYY-MM-DD"),
  }; // 변수 초기화
};
// 등록버튼 누르면 실행되는 함수
const insertDataFunction = async () => {
  insertModalData.입고코드 = addLotNum.value;
  await receive_raw.insertData(insertModalData);
...
let addLotNum = ref("");
const lot_gen = () => {
  addLotNum.value = "LOTM" + dayjs().format("YYYYMMDDHHmmss");
};
...
// 품목 데이터 설정
const url_stock_stockLot = "/api/stock/stock/lot/raw";
...
// 모달 데이터 설정
const url_stock_modal_item = "/api/stock/modal/itemraw";
const stock_modal_item = useSendApi<MasterProduct>(
  url_stock_modal_item,
  currentPage_item,
  rowsPerPage_item
);

// 테이블항목 설정 및 가로크기 조정
const table_setting_modal_item = {
  순번: { name: "순번", style: "width: 50px; text-align: center;" },
  항목1: { name: "품번", style: "width: 50px; text-align: center;" },
  항목2: { name: "구분", style: "width: 50px; text-align: center;" },
  항목3: { name: "품명", style: "width: 50px; text-align: center;" },
  항목4: { name: "차종", style: "width: 50px; text-align: center;" },
  항목5: { name: "규격", style: "width: 50px; text-align: center;" },
  항목6: { name: "단위", style: "width: 50px; text-align: center;" },
  항목7: { name: "거래처명", style: "width: 50px; text-align: center;" },
  항목8: { name: "안전재고", style: "width: 50px; text-align: center;" },
  항목9: { name: "단가", style: "width: 50px; text-align: center;" },
};
...
const importFacilitypart = (no: any) => {
  insertModalData.품목NO = no;
  insertModalData.품번 = stock_modal_item.dataAll.value.filter(
    (c) => c.NO == no
  )[0].품번;
  insertModalData.품목구분 = stock_modal_item.dataAll.value.filter(
    (c) => c.NO == no
  )[0].구분;
  insertModalData.품명 = stock_modal_item.dataAll.value.filter(
    (c) => c.NO == no
  )[0].품명;
  insertModalData.규격 = stock_modal_item.dataAll.value.filter(
    (c) => c.NO == no
  )[0].규격;
  insertModalData.단위 = stock_modal_item.dataAll.value.filter(
    (c) => c.NO == no
  )[0].단위;
...
<template>
  <div v-if="user_level >= 2">
    ...
    <div class="grid grid-cols-12 gap-1 mt-1">
      <div
        class="flex flex-wrap items-center col-span-12 mt-5 mb-2 sm:flex-nowrap"
      >
        ...
        <div class="ml-auto flex items-center mr-3">
          ...
          <div class="ml-3 relative ml-3 text-slate-500">
            <FormInput
              type="text"
              class="w-32 pr-10 !box"
              v-model="searchInput"
              @keyup.enter="
                () => {
                  search();
      ...
      <!-- BEGIN: Pagination-->
      <div class="flex flex-wrap items-center col-span-12 mt-0 sm:flex-nowrap">
        <div class="m-auto">
          <PaginationComponent
      ...
      <div class="p-3 col-span-12">
        <div v-for="(todo, index) in dataManager.datas.value" :key="todo.품번">
            ...
            <tbody class="text-center">
              ...
              <tr class="h-7 px-1">
                <td colspan="2" class="border-2 border-gray-500">
                  {{ todo[table_setting.항목4.name] }}
                </td>
                <td class="border-2 border-gray-500">
                  {{ todo[table_setting.항목5.name] }}
                </td>
              </tr>
              <tr class="h-7 px-1 font-bold bg-slate-200">
              ...
              <tr class="h-7 px-1">
                <td class="border-2 border-gray-500">
                  {{ todo[table_setting.항목11.name].toLocaleString() }}
                </td>
                <td class="border-2 border-gray-500">
              ...
              <tr>
                <td colspan="3" class="p-2 border-2 border-gray-500">
                  <Button
                    variant="primary"
                    @click="
                      () => {
  ...
  <div class="mt-5 mr-5" style="text-align: right">
  ...
  <Dialog size="md" :open="insertModal" :key="insertModalData?.품목NO">
      ...
      <div style="text-align: left">
        ...
        <div class="mt-3">
          ...
          <div class="flex">
            ...
            <div class="w-20 ml-2">
              <Button class="text-white" variant="success" @click="lot_gen()"
                >생성</Button
              >
            </div>
          </div>
        ...
        <div class="mt-3">
          ...
          <FormInput
            ...
            placeholder="이곳을 클릭하여 품목을 선택해주세요."
          />
        </div>
        <div class="mt-3">
        ...
        <div class="mt-3 flex">
          ...
          <div class="ml-1">
            <FormLabel htmlFor="vertical-form-1">입고수</FormLabel>
            <FormInput
              id="vertical-form-1"
              type="number"
        ...
        <div class="mt-3">
          <FormLabel htmlFor="vertical-form-1">유효일자</FormLabel>
          <FormInput
            id="vertical-form-1"
            type="date"
    ...
    <Dialog.Panel class="p-5 text-center" style="top: -7%">
      ...
      <div class="grid grid-cols-12 gap-1 mt-1">
        ...
        <div class="col-span-12 flex items-center">
          <div class="mr-auto">
            <span class="mr-3"
              >[ 데이터 : {{ stock_stockLot.dataCount }}개]
            </span>
            <span class="mr-4">
              [ {{ currentPage_stockLot }} /
              ...
              <table class="border-2 border-gray-500 w-full">
                <tbody class="text-center">
                  ...
                  <tr class="h-7 px-1">
                    ...
                    <td class="border-2 border-gray-500">
                      {{
                        todo[table_setting_stockLot.항목7.name].toLocaleString()
                      }}
                    </td>
                    <td class="border-2 border-gray-500">
                      {{
                        todo[table_setting_stockLot.항목8.name].toLocaleString()
                      }}
                    </td>
                  </tr>
                  <tr class="h-7 px-1 font-bold bg-slate-200">
                  ...
                  <tr class="h-7 px-1">
                    <td class="border-2 border-gray-500">
                      {{
                        todo[table_setting_stockLot.항목9.name].toLocaleString()
                      }}
                    </td>
                    <td class="border-2 border-gray-500">
                      {{
