...
import {
  MasterMoldInspect,
  MoldCheck,
} from "../../interfaces/menu/moldInterface";

// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterClientDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.PreventChangeNotify; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인 데이터 불러오기
  moldcheck_modal_moldinspect.loadDatas(); // 금형점검 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/mold/check";
const dataManager = useSendApi<MoldCheck>(url, currentPage, rowsPerPage);

// 테이블항목 설정 및 가로크기 조정
const table_setting = {
  체크박스: { name: "체크박스", style: "width: 5px" },
  순번: { name: "순번", style: "width: 5px; text-align: center;" },
  항목1: { name: "일자", style: "width: 100px; text-align: center;" },
  항목2: { name: "금형코드", style: "width: 50px; text-align: center;" },
  항목3: { name: "금형명", style: "width: 50px; text-align: center;" },
  항목4: { name: "점검구분", style: "width: 50px; text-align: center;" },
  항목5: { name: "점검내용", style: "width: 50px; text-align: center;" },
  항목6: { name: "담당자", style: "width: 50px; text-align: center;" },
  항목7: { name: "구분", style: "width: 50px; text-align: center;" },
  항목8: { name: "내용", style: "width: 50px; text-align: center;" },
...
let editModalData: MoldCheck; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
// ############################################### 금형 점검 가져오기 ###############################################
// 페이징기능
const currentPage_moldinspect = ref(1); // 현재페이지
const rowsPerPage_moldinspect = ref(10); // 한 페이지에 보여질 데이터 갯수
const pageChangeFirst_moldinspect = () => {
  currentPage_moldinspect.value = 1; // 데이터 갯수 변경 시 1페이지로 이동
};

// 품목 데이터 설정
const url_moldcheck_modal_moldinspect = "/api/mold/modal/moldinspect";
const moldcheck_modal_moldinspect = useSendApi<MasterMoldInspect>(
  url_moldcheck_modal_moldinspect,
  currentPage_moldinspect,
  rowsPerPage_moldinspect
);

// 테이블항목 설정 및 가로크기 조정
const table_setting_modal_moldinspect = {
...
// ########################## 조회기간 설정 ##########################
const searchDate_moldinspect = ref("전체기간");
// ########################## 품목 조회  ##########################
const searchKey_moldinspect = ref("전체");
const searchInput_moldinspect = ref("");
const sortKey_moldinspect = ref("등록일");
const sortOrder_moldinspect = ref("내림차순");
const sortOrderToggle_moldinspect = () => {
  sortOrder_moldinspect.value =
    sortOrder_moldinspect.value == "내림차순" ? "오름차순" : "내림차순";
};
//  정렬기준이 변경되면 실행
watch([sortKey_moldinspect, sortOrder_moldinspect], (newValue, oldValue) => {
  search_moldinspect();
  pageChangeFirst_moldinspect();
});
const search_moldinspect = () => {
  // console.log(searchKey.value, searchInput.value);
  moldcheck_modal_moldinspect.searchDatas(
    searchDate_moldinspect.value,
    searchKey_moldinspect.value,
    searchInput_moldinspect.value,
    sortKey_moldinspect.value,
    sortOrder_moldinspect.value
  );
};

// ########################## 모달 설정 ##########################
const moldinspectModal = ref(false);
const setMoldInspectModal = (value: boolean) => {
  moldinspectModal.value = value;
};

// 모달에서 선택한 품목을 itemProcesslist에 넣기
const importMoldInspect = (no: any) => {
  insertModalData.금형점검NO = no;
  insertModalData.금형코드 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].금형코드;
  insertModalData.금형명 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].금형명;
  insertModalData.점검구분 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].구분;
  insertModalData.점검내용 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].내용;
  insertModalData.검사방법 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].검사방법;
  insertModalData.검사기준 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].기준;
  insertModalData.검사단위 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].단위;
  insertModalData.기준최소 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].최소;
  insertModalData.기준최대 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].최대;
  insertModalData.담당자 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].담당자;

  editModalData.금형점검NO = no;
  editModalData.금형코드 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].금형코드;
  editModalData.금형명 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].금형명;
  editModalData.점검구분 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].구분;
  editModalData.점검내용 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].내용;
  editModalData.검사방법 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].검사방법;
  editModalData.검사기준 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].기준;
  editModalData.검사단위 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].단위;
  editModalData.기준최소 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].최소;
  editModalData.기준최대 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].최대;
  editModalData.담당자 = moldcheck_modal_moldinspect.dataAll.value.filter(
    (c) => c.NO == no
  )[0].담당자;

  setMoldInspectModal(false);
};
</script>
...
############################################################################################################################# -->
  ...
  <Dialog size="md" :open="insertModal" :key="insertModalData?.금형점검NO">
      ...
      <Tab.Group>
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            ...
            >
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">금형코드</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.금형코드"
                  @click="setMoldInspectModal(true)"
                  placeholder="여기를 클릭하여 금형점검을 등록해주세요."
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">금형명</FormLabel>
                <FormInput
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">점검구분</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.점검구분"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">점검내용</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.점검내용"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">검사방법</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.검사방법"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">검사기준</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.검사기준"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">검사단위</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.검사단위"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">기준최소</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.기준최소"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">기준최대</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.기준최대"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">담당자</FormLabel>
                <FormInput
                  type="text"
                  v-model="insertModalData.담당자"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">구분</FormLabel>
                <FormInput
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">내용</FormLabel>
                <FormInput
                  type="text"
              ...
              <div class="mt-3">
                ...
                <FormInput
                  type="text"
                  v-model="insertModalData.결과"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">일자</FormLabel>
                <FormInput
                  type="date"
  ...
  <Dialog size="md" :open="editModal" :key="editModalData?.금형점검NO">
    <Dialog.Panel class="p-10 text-center">
  ...
  <Dialog
    size="xxl"
    :open="moldinspectModal"
    @close="setMoldInspectModal(false)"
  >
    <Dialog.Panel class="p-10 text-center">
      ...
      <div class="grid grid-cols-12 gap-1 mt-1">
          ...
          <div class="w-full mt-3 sm:w-auto sm:mt-0 sm:ml-auto md:ml-2">
            <div class="relative w-56 text-slate-500">
              <FormInput
                type="text"
                class="w-56 pr-10 !box"
                v-model="searchInput_moldinspect"
                @keyup.enter="
                  () => {
                    search_moldinspect();
                    pageChangeFirst_moldinspect();
        ...
        >
          ...
          <div class="ml-3">
            <Button
              ...
              as="a"
              variant="outline-primary"
              v-if="sortOrder_moldinspect == '오름차순'"
              @click="sortOrderToggle_moldinspect"
            >
              <Lucide icon="SortAsc" class="w-4 h-4 mr-1" />

              {{ sortOrder_moldinspect }}</Button
            >
            <Button
              class="shadow-md"
              as="a"
              variant="outline-danger"
              v-if="sortOrder_moldinspect == '내림차순'"
              @click="sortOrderToggle_moldinspect"
            >
              <Lucide icon="SortDesc" class="w-4 h-4 mr-1" />
          ...
          <div class="ml-5">
            <FormSelect
              class="w-20 mt-3 !box sm:mt-0"
              v-model="rowsPerPage_moldinspect"
              @change="pageChangeFirst_moldinspect"
            >
              <option>10</option>
              <option>25</option>
              <option>50</option>
              <option>100</option>
              <option :value="moldcheck_modal_moldinspect.dataCount.value">
                전체
              </option>
            </FormSelect>
          </div>
          <div>
            <PaginationComponent
              class="pagination-component"
              v-model="currentPage_moldinspect"
              :numberOfPages="moldcheck_modal_moldinspect.numberOfPages.value"
            />
          </div>
          <div class="hidden mx-auto md:block text-slate-500"></div>
          <div>
            <span class="mr-3"
              >[ {{ moldcheck_modal_moldinspect.dataCount }}개 데이터 조회됨 ]
            </span>
            <span class="mr-4">
              [ {{ currentPage_moldinspect }} /
              {{ moldcheck_modal_moldinspect.numberOfPages }} 페이지 ]</span
            >
          </div>
        </div>
        <!-- END: Pagination-->
        ...
        <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
          ...
          >
            <Table class="border-spacing-y-[6px] border-separate -mt-2">
              ...
              >
                <Table.Tr>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.순번.style"
                  >
                    {{ table_setting_modal_moldinspect.순번.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목1.style"
                  >
                    {{ table_setting_modal_moldinspect.항목1.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목2.style"
                  >
                    {{ table_setting_modal_moldinspect.항목2.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목3.style"
                  >
                    {{ table_setting_modal_moldinspect.항목3.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목4.style"
                  >
                    {{ table_setting_modal_moldinspect.항목4.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목5.style"
                  >
                    {{ table_setting_modal_moldinspect.항목5.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목6.style"
                  >
                    {{ table_setting_modal_moldinspect.항목6.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목7.style"
                  >
                    {{ table_setting_modal_moldinspect.항목7.name }}
                  </Table.Th>
                  <Table.Th
                    class="text-center border-b-0 whitespace-nowrap"
                    :style="table_setting_modal_moldinspect.항목8.style"
                  >
                    {{ table_setting_modal_moldinspect.항목8.name }}
                  </Table.Th>
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody style="position: relative; z-index: 1">
                <Table.Tr
                  v-for="(todo, index) in moldcheck_modal_moldinspect.datas
                    .value"
                  :key="todo.NO"
                ...
                >
                  <Table.Td
                    ...
                    :style="table_setting_modal_moldinspect.순번.style"
                    @click="importMoldInspect(todo.NO)"
                  >
                    <div>
                      {{
                        index +
                        1 +
                        (currentPage_moldinspect - 1) * rowsPerPage_moldinspect
                      }}
                    </div>
                  </Table.Td>
                  <Table.Td
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목1.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목2.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목3.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목4.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목5.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목6.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목7.name] }}
                    </div>
                  </Table.Td>
                  <Table.Td
                    class="first:rounded-l-md last:rounded-r-md text-center border-b-2 dark:bg-darkmode-600"
                  ...
                  >
                    <div>
                      {{ todo[table_setting_modal_moldinspect.항목8.name] }}
                    </div>
                  </Table.Td>
                </Table.Tr>
              </Table.Tbody>
            ...
            <div
              class="text-center mt-20"
              v-if="moldcheck_modal_moldinspect.dataCount.value == 0"
            >
              저장된 데이터가 없습니다.
            </div>
          </div>
      ...
      <div style="text-align: left">
        <div class="mt-5 text-right">
          <Button
            class="mr-2 shadow-md"
            variant="outline-primary"
            @click="setMoldInspectModal(false)"
            >취소</Button
          >
        </div>
      </div>
