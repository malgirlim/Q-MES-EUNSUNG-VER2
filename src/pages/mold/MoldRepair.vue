...
import { MasterMold, MoldRepair } from "../../interfaces/menu/preventInterface";

// 컴포넌트 로드
import ProductDetail from "../../components/Common/Detail/MasterProductDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.MoldRepair; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  moldrepair_modal_mold.loadDatas(); // 금형 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/mold/repair";
const dataManager = useSendApi<MoldRepair>(url, currentPage, rowsPerPage);

// 금형 데이터
const url_moldrepair_modal_mold = "/api/mold/modal/mold";
const moldrepair_modal_mold = useSendApi<MasterMold>(
  url_moldrepair_modal_mold,
  ref(1),
  ref(10)
);
...
const table_setting = {
  ...
  항목2: { name: "금형코드", style: "width: 100px; text-align: center;" },
  ...
  항목6: { name: "결과", style: "width: 50px; text-align: center;" },
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: MoldRepair;
...
let editModalData: MoldRepair; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
            ...
            >
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.금형NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in moldrepair_modal_mold.dataAll.value"
                    :key="p.NO"
                  >
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-7">일자</FormLabel>
                <FormInput
                  id="vertical-form-7"
                  type="date"
            ...
            >
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.금형NO">
                  ...
                  >
                    {{
                      moldrepair_modal_mold.dataAll.value.filter(
