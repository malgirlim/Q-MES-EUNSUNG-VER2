...
// 컴포넌트 로드
import Detail from "../../components/Common/Detail/OrderOrderCurrentDetail.vue";
import DocumentPrint from "../../components/Common/Print/Template/Order/Main.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.OrderOrderCurrent; //권한레벨
...
const dataManager = useSendApi<OrderOrder>(url, currentPage, rowsPerPage);
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: OrderOrder;
...
let editModalData: OrderOrder; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "주문관리_발주현황_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_주문관리_발주현황.xlsx"; // 엑셀 양식주소
...
// ################################################## 수입검사요청 ##################################################
// 수입검사 데이터 설정
const url_order_incoming_request = "/api/order/order/incoming_request";
const order_incoming_request = useSendApi<QualityIncoming>(
  url_order_incoming_request,
  ref(1),
  ref(10)
);
// ########################## 검사요청 모달 설정  ##########################
// 검사요청 모달 설정
let incomingModalData: QualityIncoming = {
  발주NO: 0,
  전달사항: "",
  결과: "미검사",
}; // 등록할 변수
const incomingModal = ref(false);
const setIncomingModal = (value: boolean) => {
  if (user_level >= 3) {
    incomingModal.value = value;
  } else {
    toast.warning("액세스 권한이 없습니다.\n관리자에게 문의하세요.");
  }
};
const incomingButtonRef = ref(null);
// 요청버튼 누르면 실행되는 함수
const incomingDataFunction = async () => {
  await order_incoming_request.insertData(incomingModalData); // await : 이 함수가 끝나야 다음으로 넘어간다
  toast.success("수입검사를 요청하셨습니다.");
};
</script>
...
<template>
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-3">수주일</FormLabel>
                <FormInput
                  id="vertical-form-3"
                  type="date"
                  v-model="editModalData.수주일"
                  placeholder=""
                  readonly
                />
              </div>
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">코드순번</FormLabel>
                <FormInput
                  id="vertical-form-6"
                  type="number"
                  v-model="editModalData.코드순번"
                  placeholder=""
                  readonly
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-7">거래처명</FormLabel>
                <FormInput
                  id="vertical-form-7"
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-8">품목구분</FormLabel>
                <FormInput
                  id="vertical-form-8"
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-9">품명</FormLabel>
                <FormInput
                  id="vertical-form-9"
                  type="text"
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-10">발주수량</FormLabel>
                <FormInput
                  id="vertical-form-10"
                  type="number"
                  v-model="editModalData.발주수량"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-10">납기일</FormLabel>
                <FormInput
                  id="vertical-form-10"
                  type="date"
                  v-model="editModalData.납기일"
                  placeholder=""
                  readonly
                />
              </div>
          ...
          <Tab.Panel class="leading-relaxed">
            ...
            >
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">공급가액</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="editModalData.공급가액"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">세액</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="editModalData.세액"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">결제조건</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="editModalData.결제조건"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">결제예정일</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="date"
                  v-model="editModalData.결제예정일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">도착지주소</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="editModalData.도착지주소"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">기타</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="text"
                  v-model="editModalData.기타"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
  ...
  <Dialog
    ...
    "
    :initialFocus="incomingButtonRef"
  >
    <Dialog.Panel>
      <div class="p-5 text-center">
        <Lucide
        ...
        />
        <div class="mt-5 text-3xl">수입검사요청</div>
        <div class="mt-2 text-slate-500">정말 검사요청하시겠습니까?</div>
