...
// 컴포넌트 로드
import Detail from "../../components/Common/Detail/OrderCurrentDetail.vue";
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: OrderAccept;
...
let editModalData: OrderAccept; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "주문관리_수주현황_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_주문관리_수주현황.xlsx"; // 엑셀 양식주소
...
<template>
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-10">수량</FormLabel>
                <FormInput
                  id="vertical-form-10"
                  type="number"
  ...
  <Dialog size="xl" :open="orderModal" @close="setOrderModal(false)">
    <Dialog.Panel>
      ...
      >
        <DocumentPrint :data="editModalData" />
      </div>
    </Dialog.Panel>
  </Dialog>
  <!-- END: 수주서 확인 Modal -->
