...
// dataManager 만들기
const url = "/api/info/log";
const dataManager = useSendApi<InfoLog>(url, currentPage, rowsPerPage);

// 테이블항목 설정 및 가로크기 조정
const table_setting = {
  체크박스: { name: "체크박스", style: "width: 50px" },
  순번: { name: "순번", style: "width: 50px; text-align: center;" },
  항목1: { name: "타입", style: "width: 50px; text-align: center;" },
  항목2: { name: "메뉴", style: "width: 50px; text-align: center;" },
  항목3: { name: "내용", style: "width: 50px; text-align: center;" },
  항목4: { name: "데이터사용량", style: "width: 50px; text-align: center;" },
  항목5: { name: "등록자", style: "width: 50px; text-align: center;" },
  항목6: { name: "등록일시", style: "width: 50px; text-align: center;" },
...
let editModalData: InfoLog; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_거래처관리_" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
