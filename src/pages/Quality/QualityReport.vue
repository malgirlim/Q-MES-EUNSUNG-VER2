...
import { QualityReport } from "../../interfaces/menu/qualityInterface";

// 컴포넌트 로드
import MasterDetail from "../../components/Common/Detail/MasterQualityStdDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.QualityReport; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  qualityreport_product.loadDatas(); // 거래처 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/quality/report";
const dataManager = useSendApi<QualityReport>(url, currentPage, rowsPerPage);

// 추가 데이터
const url_qualityreport_product = "/api/quality/report/product";
const qualityreport_product = useSendApi<MasterProduct>(
  url_qualityreport_product,
  currentPage,
  rowsPerPage
);
...
const table_setting = {
  ...
  항목1: { name: "검사일", style: "width: 50px; text-align: center;" },
  ...
  항목8: { name: "검사수량", style: "width: 50px; text-align: center;" },
  항목9: { name: "샘플수량", style: "width: 50px; text-align: center;" },
  항목10: { name: "결과", style: "width: 50px; text-align: center;" },
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: QualityReport;
...
let editModalData: QualityReport; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">검사일</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="date"
                  v-model="insertModalData.검사일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="insertModalData.품목NO">
                  ...
                  <option
                    :value="p.NO"
                    v-for="p in qualityreport_product.dataAll.value"
                    :key="p.NO"
                  >
                    품번:{{ p.품번 }} # 품명:{{ p.품명 }} # 규격:{{ p.규격 }}
                  </option>
                </select>
              </div>
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">검사수량</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="number"
                  v-model="insertModalData.검사수량"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">샘플수량</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="number"
                  v-model="insertModalData.샘플수량"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">결과</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="text"
              ...
              <div class="mt-3">
                ...
                <select v-tom v-model="editModalData.품목NO">
                  <option :value="editModalData.품목NO" selected>
                    품번:{{
                      qualityreport_product.dataAll.value.filter(
      ...
      <div class="p-5 text-center">
        ...
        <div class="mt-5 mb-5">
          <img
            class="mx-auto"
            :src="
              '../../../backend/uploads/quality/report/' + editModalData.이미지
            "
          />
        </div>
        <a
