...
import { PreventLife } from "../../interfaces/menu/preventInterface";
...
onMounted(async () => {
  ...
  prevent_facilitypart.loadDatas(); // 설비부품 데이터 불러오기
...
// dataManager 만들기
const url = "/api/prevent/lifeplan";
const dataManager = useSendApi<PreventLife>(url, currentPage, rowsPerPage);
...
// 설비부품 데이터
const url_prevent_facilitypart = "/api/prevent/modal/facilitypart";
const prevent_facilitypart = useSendApi<MasterFacilityPart>(
  url_prevent_facilitypart,
  ref(1),
  ref(10)
);
...
const table_setting = {
  ...
  항목6: { name: "계획사용횟수", style: "width: 50px; text-align: center;" },
  항목7: { name: "계획교체일", style: "width: 50px; text-align: center;" },
  항목8: { name: "교체확인", style: "width: 50px; text-align: center;" },
  항목9: { name: "담당자", style: "width: 50px; text-align: center;" },
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: PreventLife;
...
let editModalData: PreventLife; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
const printPage = (data: any) => {
  printJS({
    printable: data,
    properties: ["구분", "품번", "거래처명", "품명", "규격", "단위"],
    type: "json",
    documentTitle: "기준정보 > 품목 관리",
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "기준정보_품목관리_" + dayjs().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_기준정보_품목관리.xlsx"; // 엑셀 양식주소
...
<template>
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            ...
            >
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-2">설비부품</FormLabel>
                <select v-tom v-model="insertModalData.설비부품NO">
                  <option value="" selected>=== 필수 선택 ===</option>
                  <option
                    :value="p.NO"
                    v-for="p in prevent_facilitypart.dataAll.value"
                    :key="p.NO"
                  >
                    {{ p.설비명 }} # 품번:{{ p.품번 }} # 품명:{{ p.품명 }} #
                    규격:{{ p.규격 }}
                  </option>
                </select>
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">계획사용횟수</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="number"
                  v-model="insertModalData.계획사용횟수"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-5">계획교체일</FormLabel>
                <FormInput
                  id="vertical-form-5"
                  type="date"
                  v-model="insertModalData.계획교체일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            ...
            >
              ...
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-2">설비</FormLabel>
                <select v-tom v-model="editModalData.설비부품NO">
                  <option
                    v-if="editModalData.설비부품NO"
                    :value="editModalData.설비부품NO"
                    selected
                  >
                    {{
                      prevent_facilitypart.dataAll.value.filter(
                        (c) => c.NO == editModalData.설비부품NO
                      )[0].설비명
                    }}
                    # 품번:{{
                      prevent_facilitypart.dataAll.value.filter(
                  ...
                  <option v-if="!editModalData.설비부품NO" value="" selected>
                    === 필수 선택 ===
                  </option>
                  <option
                    :value="p.NO"
              ...
              <div class="mt-3">
                ...
                <FormInput
                  id="vertical-form-5"
                  type="date"
                  v-model="editModalData.계획교체일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-3">교체확인</FormLabel>
                <FormSelect v-model="editModalData.교체확인" class="">
                  <option>Y</option>
                  <option>N</option>
                </FormSelect>
              </div>
  ...
  >
    <Dialog.Panel>
      <ProductDetail :data="editModalData" />
