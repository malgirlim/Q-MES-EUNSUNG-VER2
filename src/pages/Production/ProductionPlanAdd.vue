...
import { ProductionPlan } from "../../interfaces/menu/productionInterface";
import { OrderAccept } from "../../interfaces/menu/orderInterface";

// 컴포넌트 로드
import Detail from "../../components/Common/Detail/ProductionPlanAddDetail.vue";

const { proxy }: any = getCurrentInstance();
const user_level = proxy.gstate.level.ProductionPlanAdd; //권한레벨

// 페이지 로딩 시 시작
onMounted(async () => {
  dataManager.loadDatas(); // 메인으로 쓸 데이터 불러오기
  plan_accept.loadDatas(); // 수주 데이터 불러오기
});
...
// dataManager 만들기
const url = "/api/production/plan";
const dataManager = useSendApi<ProductionPlan>(url, currentPage, rowsPerPage);

// 추가 데이터
const url_plan_accept = "/api/production/plan/accept";
const plan_accept = useSendApi<OrderAccept>(
  url_plan_accept,
  currentPage,
  rowsPerPage
);
...
const table_setting = {
  ...
  항목8: { name: "계획시작일", style: "width: 50px; text-align: center;" },
  항목9: { name: "계획종료일", style: "width: 50px; text-align: center;" },
...
// ########################## 등록, 수정, 삭제, 상세 Modal ##########################
// ##### 등록 Modal #####
let insertModalData: ProductionPlan;
...
let editModalData: ProductionPlan; // 수정할 변수
// 수정버튼 누르면 실행되는 함수
...
function exportFile(data: any) {
  ...
  writeFileXLSX(
    wb,
    "생산관리_생산계획" + moment().format("YYMMDD_HHmmss") + "_export.xlsx"
  );
}
...
const onFileImportForm =
  "../../src/assets/xlsx/업로드양식_생산관리_생산계획.xlsx"; // 엑셀 양식주소
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-2">수주</FormLabel>
                <select v-tom v-model="insertModalData.수주NO">
                  <option value="" selected>=== 추가 선택 ===</option>
                  <option
                    :value="p.NO"
                    v-for="p in plan_accept.dataAll.value"
                    :key="p.NO"
                  >
                    {{ p.코드 }} # 순번:{{ p.코드순번 }} # 품번:{{ p.품번 }} #
                    품명:{{ p.품명 }} # 수량:{{ p.수량 }}
                  </option>
                </select>
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-4">계획수량</FormLabel>
                <FormInput
                  id="vertical-form-4"
                  type="number"
                  v-model="insertModalData.계획수량"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">계획시작일</FormLabel>
                <FormInput
                  id="vertical-form-6"
                  type="date"
                  v-model="insertModalData.계획시작일"
                  placeholder=""
                />
              </div>
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-6">계획종료일</FormLabel>
                <FormInput
                  id="vertical-form-6"
                  type="date"
                  v-model="insertModalData.계획종료일"
                  placeholder=""
                />
              </div>
            </div>
        ...
        <Tab.Panels class="mt-5">
          <Tab.Panel class="leading-relaxed">
            <div style="text-align: left">
              <div class="mt-3">
                <FormLabel htmlFor="vertical-form-2">수주</FormLabel>
                <select v-tom v-model="editModalData.수주NO">
                  <option
                    v-if="editModalData.수주NO"
                    :value="editModalData.수주NO"
                    selected
                  >
                    {{
                      plan_accept.dataAll.value.filter(
                        (c) => c.NO == editModalData.수주NO
                      )[0].코드
                    }}
                    # 순번:{{
                      plan_accept.dataAll.value.filter(
                  ...
                  <option v-if="!editModalData.수주NO" value="" selected>
                    === 추가 선택 ===
                  </option>
                  <option
                    :value="p.NO"
