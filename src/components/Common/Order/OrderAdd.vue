...
onMounted(() => {
  ...
  accept_client.loadDatas(); // 거래처 데이터 불러오기
});

// 수주 데이터 설정
const url_accept = "/api/order/accept";
const accept = useSendApi<OrderAccept>(url_accept, ref(1), ref(10));

const orderAcceptInit: OrderAccept = {
  NO: undefined,
  수주일: undefined,
  코드: undefined,
  코드순번: undefined,
...
// 품목 리스트를 저장할 곳
// const items = reactive<OrderAccept[]>([{ ...orderAcceptInit }]);
let items = ref<OrderAccept[]>([{ ...orderAcceptInit, NO: 1 }]);
...
const addRows = () => {
  ...
  items.value.push({ ...orderAcceptInit, NO: maxNO + 2 });
};
...
const insertData = async () => {
  await items.value.forEach((item) => {
    item.수주일 = defaultData.value.수주일;
    item.코드 = defaultData.value.코드;
    item.거래처NO = defaultData.value.거래처NO;
    item.연락처 = defaultData.value.연락처;
    item.결제조건 = defaultData.value.결제조건;
    item.결제예정일 = defaultData.value.결제예정일;
    item.납기일 = defaultData.value.납기일;
    item.도착지주소 = defaultData.value.도착지주소;
    item.기타 = defaultData.value.기타;
    item.공급가액 = String(Number(item.수량) * Number(item.단가));
    item.세액 = String((Number(item.수량) * Number(item.단가)) / 10);
  });
  await accept.insertAllData(items.value);
  router.push("/order/current");
};
...
const importClient = (no: any) => {
  ...
  defaultData.value.연락처 = accept_client.dataAll.value.filter(
    (c) => c.NO == no
  )[0]?.전화번호;
  defaultData.value.도착지주소 = accept_client.dataAll.value.filter(
    (c) => c.NO == no
  )[0]?.주소;
...
// 거래처 데이터 설정
const url_accept_client = "/api/order/accept/client";
const accept_client = useSendApi<MasterClient>(
  url_accept_client,
  currentPage_client,
  rowsPerPage_client
);
...
const table_setting_client = {
  ...
  항목3: { name: "거래처명", style: "width: 100px; text-align: center;" },
  항목4: { name: "주소", style: "width: 250px; text-align: left;" },
...
const importItem = (no: any) => {
  ...
  items.value.filter((c) => c.NO === itemModalIndex.value)[0].단가 =
    accept_product.dataAll.value.filter((c) => c.NO == no)[0]?.단가;
...
// 품목 데이터 설정
const url_accept_product = "/api/order/accept/product";
const accept_product = useSendApi<MasterProduct>(
  url_accept_product,
  currentPage_item,
  rowsPerPage_item
);
          ...
          <Table.Tr class="intro-y">
            ...
            <Table.Td class="px-1 py-1">
              <FormInput
                ...
                formInputSize="sm"
                :value="
                  accept_client.dataAll.value.filter(
          ...
          <Table.Tr class="intro-y">
            ...
            <Table.Td class="px-1 py-1">
              <FormInput
                type="text"
                formInputSize="sm"
                :value="defaultData.연락처"
                v-model="defaultData.연락처"
              ></FormInput>
            </Table.Td>
          </Table.Tr>
        </Table.Tbody>
        ...
        <Table.Tbody>
          <Table.Tr class="intro-y">
            ...
            <Table.Td
              ...
              ><strong>도착지 주소</strong>
            </Table.Td>
            <Table.Td class="px-1 py-1 text-center" style="width: 350px">
              <FormInput
                type="text"
                formInputSize="sm"
          ...
          <Table.Tr class="intro-y">
            ...
            <Table.Td class="px-1 py-1 text-center">
              <FormSelect formSelectSize="sm" v-model="defaultData.결제조건">
                <option>현금</option>
                <option>카드</option>
                <option>외상</option>
                <option>어음</option>
            ...
            <Table.Td class="px-1 py-1 text-center" rowspan="2">
              <FormTextarea
                formTextareaSize="sm"
                style="height: 63px"
          ...
          <Table.Tr class="intro-y">
            <Table.Td
              class="px-1 py-1 text-center bg-gray-300"
              style="border-right-width: 1px"
              ><strong>결제 예정일</strong>
            </Table.Td>
            <Table.Td class="px-1 py-1 text-center">
              <FormInput
                type="date"
                formInputSize="sm"
                v-model="defaultData.결제예정일"
              ></FormInput>
            </Table.Td>
          </Table.Tr>
        </Table.Tbody>
          ...
          <Table.Tr v-for="(item, index) in items" :key="item.NO">
            ...
            <Table.Td
              ...
              ><FormInput
                ...
                formInputSize="sm"
                :value="
                  accept_product.dataAll.value.filter(
                    (c) => c.NO === item.품목NO
                  )[0]?.품번
                "
                readonly
            ...
            <Table.Td
              ...
              ><FormInput
                type="text"
                formInputSize="sm"
                :value="(Number(item.수량) || 0) * (Number(item.단가) || 0)"
                readonly
              ></FormInput
            ></Table.Td>
            <Table.Td
              ...
              ><FormInput
                ...
                formInputSize="sm"
                :value="
                  ((Number(item.수량) || 0) * (Number(item.단가) || 0)) / 10
                "
                readonly
              ></FormInput
            ></Table.Td>
        ...
        >
          ...
          <div class="ml-5">
            ...
            >
              ...
              <option>50</option>
              <option>100</option>
              <option :value="accept_client.dataCount.value">전체</option>
            </FormSelect>
          </div>
          <div>
            <PaginationComponent
              ...
              :numberOfPages="accept_client.numberOfPages.value"
            />
          </div>
          <div class="hidden mx-auto md:block text-slate-500"></div>
          <div>
            <span class="mr-3"
              >[ {{ accept_client.dataCount }}개 데이터 조회됨 ]
            </span>
            <span class="mr-4">
              [ {{ currentPage_item }} /
              {{ accept_client.numberOfPages }} 페이지 ]</span
            >
          </div>
        </div>
        <!-- END: Pagination-->
          ...
          >
            <Table class="border-spacing-y-[6px] border-separate -mt-2">
              ...
              <Table.Tbody style="position: relative; z-index: 1">
                <Table.Tr
                  v-for="(todo, index) in accept_client.datas.value"
                  :key="todo.NO"
            ...
            <div
              ...
              v-if="accept_client.dataCount.value == 0"
            >
              저장된 데이터가 없습니다.
            </div>
          </div>
        ...
        >
          ...
          <div class="ml-5">
            ...
            >
              ...
              <option>50</option>
              <option>100</option>
              <option :value="accept_product.dataCount.value">전체</option>
            </FormSelect>
          </div>
          <div>
            <PaginationComponent
              ...
              :numberOfPages="accept_product.numberOfPages.value"
            />
          </div>
          <div class="hidden mx-auto md:block text-slate-500"></div>
          <div>
            <span class="mr-3"
              >[ {{ accept_product.dataCount }}개 데이터 조회됨 ]
            </span>
            <span class="mr-4">
              [ {{ currentPage_item }} /
              {{ accept_product.numberOfPages }} 페이지 ]</span
            >
          </div>
        </div>
        <!-- END: Pagination-->
        ...
        <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
          ...
          >
            <Table class="border-spacing-y-[6px] border-separate -mt-2">
              ...
              <Table.Tbody style="position: relative; z-index: 1">
                <Table.Tr
                  v-for="(todo, index) in accept_product.datas.value"
                  :key="todo.NO"
                  class="intro-x hover:bg-gray-200 active:bg-gray-300 cursor-pointer"
                >
                  <Table.Td
            ...
            <div
              class="text-center mt-20"
              v-if="accept_product.dataCount.value == 0"
            >
              저장된 데이터가 없습니다.
            </div>
          </div>
