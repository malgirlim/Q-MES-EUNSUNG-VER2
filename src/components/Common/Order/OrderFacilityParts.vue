...
// 페이지 로딩 시 시작
onMounted(() => {
  order_facilityPart.loadDatas(); // 품목 데이터 불러오기
  order_client.loadDatas(); // 거래처 데이터 불러오기
});

// 설비부품발주 데이터 설정
const url_facilitypart = "/api/order/facilitypart";
const facilitypart = useSendApi<OrderFacilityPart>(
  url_facilitypart,
  ref(1),
  ref(10)
);

const orderFacilityPartInit: OrderFacilityPart = {
  NO: undefined,
  발주코드: undefined,
  발주구분: undefined,
  발주일자: undefined,
...
const defaultData = ref({
  ...orderFacilityPartInit,
  발주일자: dayjs().format("YYYY-MM-DD"),
});

// 품목 리스트를 저장할 곳
// const items = reactive<OrderAccept[]>([{ ...orderFacilityPartInit }]);
let items = ref<OrderFacilityPart[]>([{ ...orderFacilityPartInit, NO: 1 }]);
...
const addRows = () => {
  ...
  items.value.push({ ...orderFacilityPartInit, NO: maxNO + 2 });
};
...
const insertData = async () => {
  ...
  await facilitypart.insertAllData(items.value);
  router.push("/order/facility-parts-current");
};
...
const importClient = (no: any) => {
  ...
  order_facilityPart.searchDatas(
...
// 거래처 데이터 설정
const url_order_client = "/api/order/facilitypart/client";
...
const importItem = (no: any) => {
  ...
  items.value.filter((c) => c.NO === itemModalIndex.value)[0].단가 =
    order_facilityPart.dataAll.value.filter((c) => c.NO == no)[0]?.단가;
...
// 품목 데이터 설정
const url_order_facilityPart = "/api/order/facilitypart/part";
const order_facilityPart = useSendApi<MasterFacilityPart>(
  url_order_facilityPart,
  currentPage_item,
  rowsPerPage_item
);
          ...
          <Table.Tr v-for="(item, index) in items" :key="item.NO">
            ...
            <Table.Td
              ...
              ><FormInput
                ...
                formInputSize="sm"
                :value="
                  order_facilityPart.dataAll.value.filter(
                    (c) => c.NO === item.품목NO
                  )[0]?.품번
                "
                readonly
        ...
        >
          ...
          <div class="ml-5">
            ...
            >
              ...
              <option>50</option>
              <option>100</option>
              <option :value="order_facilityPart.dataCount.value">전체</option>
            </FormSelect>
          </div>
          <div>
            <PaginationComponent
              ...
              :numberOfPages="order_facilityPart.numberOfPages.value"
            />
          </div>
          <div class="hidden mx-auto md:block text-slate-500"></div>
          <div>
            <span class="mr-3"
              >[ {{ order_facilityPart.dataCount }}개 데이터 조회됨 ]
            </span>
            <span class="mr-4">
              [ {{ currentPage_item }} /
              {{ order_facilityPart.numberOfPages }} 페이지 ]</span
            >
          </div>
        </div>
        <!-- END: Pagination-->
        ...
        <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
          ...
          >
            <Table class="border-spacing-y-[8px] border-separate -mt-2">
              ...
              <Table.Tbody style="position: relative; z-index: 1">
                <Table.Tr
                  v-for="(todo, index) in order_facilityPart.datas.value"
                  :key="todo.NO"
                  class="intro-x hover:bg-gray-200 active:bg-gray-300 cursor-pointer"
                >
                  <Table.Td
            ...
            <div
              class="text-center mt-20"
              v-if="order_facilityPart.dataCount.value == 0"
            >
              저장된 데이터가 없습니다.
            </div>
          </div>
